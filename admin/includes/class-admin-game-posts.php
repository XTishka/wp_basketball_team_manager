...
class Admin_Game_Posts extends Basketball_Team_Manager_Admin {
	...
	public function register_sponsors_taxonomy() {
		...
		register_taxonomy( 'sponsors', array( 'bt-games', 'bt-players' ), array(
			'hierarchical'      => false,
			'labels'            => $labels,
			'show_ui'           => true,
			'show_admin_column' => false,
	...
	public function game_meta_box_callback( $post, $meta ) {
		...
		$gameData = array(
			...
			'calendar_event_id' => get_post_meta( $post->ID, 'game_calendar_event_id', 1 ),
		);

		$teams           = array();
		$teamsTaxonomies = get_the_terms( $post->ID, 'teams' );
		if ( ! empty( $teamsTaxonomies ) ) {
			foreach ( $teamsTaxonomies as $taxonomy ) {
				array_push( $teams, $taxonomy->name );
			}
		}

		$arenas           = array();
		$arenasTaxonomies = get_the_terms( $post->ID, 'arenas' );
		if ( ! empty( $arenasTaxonomies ) ) {
			foreach ( $arenasTaxonomies as $taxonomy ) {
				array_push( $arenas, $taxonomy->name );
			}
		}

		$tournaments           = array();
		$tournamentsTaxonomies = get_the_terms( $post->ID, 'tournaments' );
		if ( ! empty( $tournamentsTaxonomies ) ) {
			foreach ( $tournamentsTaxonomies as $taxonomy ) {
				array_push( $tournaments, $taxonomy->name );
			}
		}

		$tv_channels           = array();
		$tv_channelsTaxonomies = get_the_terms( $post->ID, 'tv_channels' );
		if ( ! empty( $tv_channelsTaxonomies ) ) {
			foreach ( $tv_channelsTaxonomies as $taxonomy ) {
				array_push( $tv_channels, $taxonomy->name );
			}
		}
		...
		game_data_form( $gameData, $this->plugin_name, $teamsTerms, $arenasTerms, $seasonsTerms, $tournamentsTerms, $tvTerms, $sponsorsTerms, $arenas, $tournaments, $tv_channels, $teams );
	...
	public function save_game_post( $post_id ) {
		...
		if ( isset( $_POST ) and $post_type == 'bt-games' ) {
			...
			$calendar = new GoogleCalendarAdmin();

			if ( empty( $_POST['game_calendar_event_id'] ) ) {
				$event = $calendar->createEvent( $_POST );
				update_post_meta( $post_id, 'game_calendar_event_id', $event->id );
			} else {
				$calendar->updateEvent( $_POST );
			}
		}
	}
